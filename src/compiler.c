...
static void endScope() {
    ...
    while (current->localCount > 0 &&
        ...
        // Challenge: implement OP_POPN to pop N variables at once.
        emitByte(OP_POP);
        current->localCount--;
    }
}

// forward declarations to help with circular definitions
static void parsePrecedence(Precedence precedence);

static uint8_t identifierConstant(Token* name) {
    // Creates a constant in the constant table and return the index
    // of the constant.
...
static void declareVariable() {
    ...
    for (int i = current->localCount - 1; i >= 0; i--) {
        // Check for a local variable of the same name.
...
static void returnStatement() {
    if (current->type == TYPE_SCRIPT) {
        // Prevent a return statement in the top-level script.
...
static void whileStatement() {
    // store the location that we'll jump back to, ensuring we always
